...
**Last updated:** 23 July 2026 (review round 5 applied — LAUNCH CANDIDATE, pending operational sign-off)
...
>
> **Round 4 (final):** SBX-R3-Q4 replaced again (still duplicated Q1's lesson) with the sensory-description principle; HOS-R3-Q1 resequenced so "check they need nothing else" precedes the thanks/invite; RET-R3-Q1 and Q3 now include the payment step (both were checkout journeys with no payment); R1 renamed "Radar Mode: Read the Moment" (the old "First 7 Seconds" didn't fit scenes where the customer had been browsing for minutes) and its intro reworded to match; final Arabic fixes (بأي لحظة للوقت، وأأكّد، على اطلاع وقت الزحمة، RET-R5-Q5 Arabic feedback which still carried the pre-round-3 message).
>
> **Round 5 (final copy):** RET-R3-Q1 feedback softened and aligned with the new payment sequence; RET-R2-Q1 ("they buy their life") and RET-R3-Q6 ("wrapping isn't a service") reworded for accuracy; HOS-R3-Q5 softened to "can influence". The SBX-R4-Q4 Arabic typo the reviewer flagged was ALREADY fixed in rev5 — they were reading the previous document. **Always confirm the reviewer has the latest docx before actioning.**
>
> **⛔ PRE-LAUNCH GATE — operational sign-off (NOT done):** 24 of the 94 questions teach an *approved action* rather than a service attitude, and need Ops/Brand confirmation. The generated docx now contains a dedicated sign-off page listing them by area with tick-boxes, and each affected question is flagged "⚑ needs ops sign-off" inline. Areas: Aura/loyalty timing (7) · price correction & supervisor authorization (1) · returns/exchange of opened beauty products (1) · tester & sample hygiene (6) · complimentary replacement & compensation (7) · birthday touches & recovery gestures (2) · Starbucks customization & remake wording (2).
>
...
**Stale auto-fill when the Emp ID changes (fixed 23 July):** entering a second ID left the FIRST person's name in the field while the green confirmation showed the new person. Cause: the "never overwrite their typing" guard could not distinguish a value the player typed from one the app had auto-filled earlier. Fixed with provenance tracking — `nameEl.dataset.autoName` records what WE set, and `state.brandAuto` does the same for the brand. On a new lookup `clearAutoFilled()` removes only app-set values. A player's own typed name or manually chosen brand is always preserved, including across an ID correction. Late responses for a superseded ID are discarded (`if (v !== lastLooked) return`).

**Brand naming: display vs storage (decided 23 July).** The dropdown and badge still show the player their OWN language (ستاربكس / Starbucks) — Arabic brand names stay, because Latin text inside an RTL screen reads badly and the 94 questions already use transliterated names. But what is SAVED is always the **canonical English name plus the 3-letter company code** (`brand: "Starbucks", brandCode: "STA"`), so one brand is one row in every pivot no matter which language the player chose. All 50 brands resolve to a code. `BrandCode` was appended as the LAST column in Scores and Feedback deliberately — inserting it next to Brand would have shifted ~15 hardcoded column indices across the backend, dashboard and completion report.

**"Cannot call SpreadsheetApp.getUi() from this context" (fixed 23 July):** `getUi()` only exists when the script is bound to a sheet AND run from a UI context; it threw at the END of `setup()`, on the cosmetic confirmation popup, AFTER all tabs had already been created — so the setup had actually worked. Fixed properly: every alert now goes through `say_()` (popup if a UI exists, otherwise `Logger.log`, never throws), the menu build in `onOpen()` is wrapped in try/catch, and all 10 spreadsheet lookups route through `ss_()`. A new optional `SHEET_ID` constant at the top lets a STANDALONE script work too — leave it empty for a bound script. Verified across four contexts: bound+UI, bound without UI, standalone (clear actionable error), standalone with SHEET_ID.

**Apps Script cold start (fixed 23 July):** the container sleeps after inactivity and the FIRST request then takes ~5s. Symptom was "first ID entry never fetches, works after a page refresh". Three causes, all fixed: (1) `warmBackend()` now pings the script on page load so it is hot before anyone types; (2) `fetchJson()` gives every call an 8s timeout plus ONE automatic retry; (3) the `lastLooked` guard is cleared when a call actually fails, so re-entering the same ID retries instead of being silently ignored until reload. The name field is now released after 1.8s but the request continues, and a late result still fills any field the player has not typed into.

**Intake lookup timing (fixed 23 July):** the roster lookup fires on a **debounced `input`** (350ms) while the player is still typing their Emp ID, NOT on `blur` — blur fired at the exact moment they clicked into the name field, so the answer always arrived after they had started typing. While a lookup is in flight the name field is held `readOnly` with a "fetching" placeholder; a hard **2.5s timeout always releases it**, so a slow or dead network can never trap the player. A typed name is never overwritten.
